...
\begin{itemize}
	\item \textit{Representing the relationships} among projects and topics retrieved from 
...
As shown in Fig. \ref{fig:CrossRecArchitecture}, the developer interacts with the system by sending a request for recommendations \circled{1}. 
Such a request contains a list of topics that are already included in the project the developer is working on. 
The \code{Data Encoder} \circled{4} collects \emph{background data} from OSS 
repositories \circled{5}, represents them in a graph, which is then used as a 
base for other components of \CR. The \code{Similarity Calculator}  
module \circled{2} computes similarities among projects to find the most 
similar ones to the given project. The \code{Recommendation Engine} \circled{3} 
implements a \emph{collaborative-filtering} technique 
\cite{Aggarwal2016},\cite{Zhao:2010:UCR:1748610.1749278}, it selects top-$k$ similar  projects, and performs computation to generate a ranked list of \emph{top-N} 
topics.~Finally, the recommendations \circled{6} are sent back to the developer. 
...
%\revised{
%It is worth noting that \CR recommends a library, both with or without a version, depending on the availability of the training data. In this way, our proposed approach transcends the limitation of the baselines considered in this paper, \ie~\LF, \LR and \LC. Such approaches are not capable of recommending a specific version of a library.}

%it does not recommend a specific version of such a library. Recommending library versions will be the subject of our future work.


%\footnote{To facilitate future research, we will publish the \CR source 
%code together with the evaluation dataset in the camera-ready version}




%\textbf{[??? Add some words about the implementation of \CR and add the reference]}

In the following, the components \code{Data Encoder}, \code{Simila\-rity Calculator}, and \code{Recommendat\-ion Engine} are singularly described. % shown in Fig. \ref{fig:CrossRecArchitecture} \cite{CROSSREC-DATA}
...
The \code{Recommendation Engine} of \CR works by relying on an analogous user-item ratings matrix. To provide 
...
%This reflects the actual relationship of the projects by the view of Software Engineering: they are similar since they implement common functionalities by using common libraries .
%By the view of recommendation algorithms, given a project $p$, suggested libraries for $p$ should come from the most similar projects to $p$ \cite{DBLP:conf/rweb/NoiaO15}. %Since VsmSim quantifies similarities by using libraries per se, it is our firm belief that the technique will facilitate the search for similar projects with respect to dependency usage.

%Khrouf:2013:HER:2507157.2507171,

%in terms of
%By VsmSim\footnote{For the sake of clarity, the name is given by ourselves since the metric was not name by its authors}, 
%In this sense, it is highly important to find the most similar projects to $p$. Since  In this way,  Vector Space Model.  the nodes connected to it represented using a vector as follows
%Being based on the Vector Space Model  \cite{Turney:2010:FMV:1861751.1861756}, 
% with each entry corresponding to the weight or a neighbour node:
