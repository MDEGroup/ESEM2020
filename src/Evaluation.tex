...
It is worth noting that we can't directly compare directly \CT and \MNB approaches because they rely on different data as input (\ie \CT requires an initial set of assigned topics for suggesting new topics, whereas\MNB uses the information mined from the README files). 
\JDR{Check if the entangled approach is described somewhere}
