...
\begin{figure}[t!]
	...
	\label{fig:configs}
\end{figure}
As we are relying on a collaborative filtering technique, the number of output topics, the number of neighbours, and the data preprocessing play an important role in the assessment. Thus, we variate  the recommended list of topics \emph{N} for 5 and 10, and the number of neighbours \emph{k} \ie \emph{N} = \{5,10,15,20,25\}. Moreover, we use different frequency topic cut-off \emph{t} to remove very infrequent topics from the dataset. The bar charts in Figure \ref{fig:success-rateN5} and \ref{fig:success-rateN10} show the success rate outcomes of \CT, divided by the\emph{t}. Figure \ref{fig:success-rateN5} and \ref{fig:success-rateN10} shows the success rate considering the first 5 and 10 recommended topics respectively. The horizontal axes shows the success rate outcomes for different size of neighbours \emph{N}.
...
\centering
	...
	\\
	\subfigure[Success rate@5]{\label{fig:RecallRate5}
	\includegraphics[width=0.45\linewidth]{figs/successRateN@5.pdf}} &
	\subfigure[Success rate@10]{\label{fig:RecallRate10}
	\includegraphics[width=0.45\linewidth]{figs/successRateN@10.pdf}}
	\end{tabular} 
...
For each repository \emph{p}, the evaluation outcomes consider the half part of real topic as input topics value and remaining ones as ground truth data \emph{GT(p)}. In our evaluation we are also interested to understand how the number of input topics impacts on prediction performance. Figure~\ref{fig:pr-input-topics} shows the success rate by choosing different number of input topics. Varying \emph{|t$_{in}$|} means changing the length of input topics that enable the \CT collaborative filtering recommender. In this picture we report the success values for the best configuration settings (\ie \emph{k} = 25, \emph{t} = 20 and) . The success rate values exhibits an improvements when the size of input topic rises. This  behaviour demonstrate that \CT computes better similar project when it has a higher number of  
This is due to the similarity function that has been involved in the computation of first \emph{k} neighbours. Hi
\begin{figure}[t!]
	\centering
...
We evaluate both approaches by variating the number of input topics up to 20. For the sake of the presentation, we report half of the data as we aim to show the overall trend.
