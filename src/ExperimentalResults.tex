...
Catalog coverage measures the percentage of recommended topic in the training data that the model is able to recommend on a test set, and a higher value corresponds to a better coverage. In our experiment, we measured the catalog coverage value among topics, \ie 15,743. Table~\ref{tab:coverage} reports the average coverage value for the considered datasets, \ie $D_{1}$, $D_{5}$, $D_{10}$, $D_{15}$, $D_{20}$). %all ten rounds. 
...
As already reasoned in Section~\ref{sec:methodology-metric}, it is not feasible to directly compare \TF with \MNB, as they rely on different input data. \TF is a supervised learning system that requires an initial set of assigned topics for the training, whilst \MNB is unsupervised learning and it uses only information mined from README files to recommend, without making use of any topics as input. However, we can compare.
