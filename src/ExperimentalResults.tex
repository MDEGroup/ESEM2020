...
As we are relying on a collaborative filtering technique, the number of output topics, the number of neighbours, and the data preprocessing play an important role in the assessment. Thus, we variate  the recommended list of topics \emph{N} for 5 and 10, and the number of neighbours \emph{k} \ie \emph{N} = \{5, 10, 15, 20, 25\}. Moreover, the figure depicts the results of \CT applied on the dataset mined defined in Section~\ref{sec:Dataset} \ie $Dt_{1}$, $Dt_{5}$, $Dt_{10}$, $Dt_{15}$, and $Dt_{20}$.
...
As defined in Section~\ref{sec:metrics}, the coverage metric is the percent of recommended topic in the training data that the model is able to recommend on a test set. Table\ref{tab:coverage} reports the average coverage value of all ten folds.
For each dataset (\ie $Dt_{1}$, $Dt_{5}$, $Dt_{10}$, $Dt_{15}$, $Dt_{20}$), 
the table consists of two coverage values: the former (\ie \emph{Dataset coverage}) is computed by considering the coverage with respect to the number of topic after the filtering step, whereas the latter considers all mined topics (\ie 15,743 topics). The value of \emph{Dataset coverage} grows by increasing the topic frequency cut-off value \emph{t}, whereas the \emph{Global coverage} decreases because there are no training data to recommend infrequent topics. The average \emph{Global coverage} values decrease from 9.306\% (Dt$_1$) to 1.805\% (Dt$_{20}$).
Having a higher value of \emph{t}, it strongly impacts on the global catalog coverage value, because too many training data are discarded due to the topic frequency cut-off. Differently from the discussed metric outcomes, this exeriment shows how an higher values of topic frequency cut-off negatively impacts on the global catalog coverage value.
...
\begin{figure}[t!]
	...
	\caption{Evaluation of the different input topics. \CDS{Check if we want to keep this or just delete}}
