...
\subsection{Entanglement with \MNB} \label{sec:entangled_approach}

So far, we have described \CT as a stand-alone recommender system by detailing all the involved components in the process. To highlight its flexibility in a different context, we \emph{entangle} our tool with the MNB network using it as a black box. In this section, we show how the \CT approach can be combined with other recommendation systems that use, for instance, a different mining schema.
...
\begin{figure}
	...
	\caption{Overview of the entangled approach}
	\label{fig:entangled}
\end{figure}
