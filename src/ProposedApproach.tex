...
In the following subsections, we describe two \TF configurations, namely \TFa and \TFb to recommend relevant topics by incorporating different types of input data. \TFa exploits a collaborative-filtering technique, while \TFb is a combination of both \MNB and \TFa, aiming to improve the prediction performance of \MNB. % by combining it with \TFa. %\TF can be combined with \MNB to improve. %the collaborative filtering prediction performance.
...
\begin{itemize}
	...
	\item \TFb recommends topics by iterating over refining steps: once we select some topics from the recommended ones, \TFb can discover new topics; using the selected one as new input;
