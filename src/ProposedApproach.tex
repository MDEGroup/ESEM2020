...
Furthermore, based on the proposed architecture, we present an approach to improve \MNB by combining with the collaborative-filtering technique. %\TF can be combined with \MNB to improve. %the collaborative filtering prediction performance.
...
Each row in the matrix represents a project, the items are represented as columns, and each cell in the matrix corresponds to a rating given by a user for an item
...
So far, we have described \TF as a stand-alone recommender system by describing its constituent components. To highlight its flexibility in a different context, we propose an approach to improve \MNB. %as follows. %\emph{entangle} our tool with the MNB network using it as a black box. 
