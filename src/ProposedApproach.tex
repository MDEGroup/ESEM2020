...
\TF is a recommender system %\cite{Aggarwal2016} 
...
Similarly, the \code{Recommendation Engine} of \TF relies on the information extracted from very similar project to recommend relevant topics to the input repository. 
...
So far, we have described \TF as a stand-alone recommender system by detailing all the involved components in the process. To highlight its flexibility in a different context, we propose an approach to improve the existing \MNB as follows. %\emph{entangle} our tool with the MNB network using it as a black box. 
In this section, we show how \TF can be combined with other recommendation systems that use, for instance, a different mining schema.
...
\begin{figure}
	...
	\label{fig:entangled}
\end{figure}
Figure~\ref{fig:entangled} depicts an overview of the entangled approach. First, the \MNB computes the list of recommended featured topics from the repository readme file. Then, top-N featured topics are used as input to \TF that computes their topics  recommendation (\ie non-featured topics are included). Finally, the recommended topics are mixed by encoding the \TF recommended topics to the top-N featured ones.

In our belief, \TF could improve \MNB in the following term:
\begin{itemize}
	\item \TF recommends non featured topics: the recommended topics are selected from similar repository independently from their nature;
	...
	\item \MNB doesn't provide further recommendations if the suggested topics are already included, whereas we will see in Section~\ref{sec:EXP1} \TF performs better when more topics are used as input.
\end{itemize}
The experiments conducted in the followings sections aim to evaluate the performance of \TF, \MNB, and the entangled approach by comparing the accuracy and coverage metrics.
